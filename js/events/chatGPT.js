    ...
    if (process.env.MODE !== "no-gpt" && process.env.MODE !== "local") {
      res = await api
        ...
        .then((res) => {
          ...
          return api.sendMessage(
            `забудь про json. Добавь в начало поста следующую информацию, не добавляй слов "категория, стоимость, дата, время, место". переведи при необходимости на русский язык: ${(category && category !== "") || (categoryGPT && categoryGPT !== "") ? `\n🏷️ ${category || categoryGPT}` : ""
